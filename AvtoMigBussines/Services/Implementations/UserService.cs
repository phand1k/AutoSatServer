...
namespace AvtoMigBussines.Services.Implementations
{
    public class UserService : IUserService
    {
        ...
        public UserService(IUserRepository userRepository, UserManager<AspNetUser> userManager, IConfiguration configuration, IOrganizationRepository organizationRepository, IOrganizationService organizationService)
        {
        ...
        public async Task VerifyOTP(string? phoneNumber, string? code)
        {
            var client = new HttpClient();
            var request = new HttpRequestMessage
            {
                Method = HttpMethod.Get,
                RequestUri = new Uri($"https://whatsauth-whatsapp-otp.p.rapidapi.com/verify-otp?phone={phoneNumber}&otp={code}"),
                Headers =
                {
                    { "x-rapidapi-key", "011743526bmsh6436c39e33bcaf8p1e7774jsn0a264ff79da7" },
        ...
        public async Task SendOTP(string? phoneNumber)
        {
            ...
            {
                Method = HttpMethod.Get,
                RequestUri = new Uri($"https://whatsauth-whatsapp-otp.p.rapidapi.com/send-otp/?phone={phoneNumber}&length=4&expiry=10&company=AutoSat"),
                Headers =
                {
                    { "x-rapidapi-key", "011743526bmsh6436c39e33bcaf8p1e7774jsn0a264ff79da7" },
                    { "x-rapidapi-host", "whatsauth-whatsapp-otp.p.rapidapi.com" },
        ...
        public async Task RegisterUserWithOutOrganizationNumberAsync(RegisterWithoutOrganizationModel model, int? typeOfOrganizationId)
        {
            var userExists = await userManager.FindByNameAsync(model.PhoneNumber);
            ...
            var organizationId = await organizationService.CreateAutomaticOrganization(typeOfOrganizationId);
            ...
            var result = await userManager.CreateAsync(aspNetUser, model.Password);
            await userManager.AddToRoleAsync(aspNetUser, "Директор");
        }
