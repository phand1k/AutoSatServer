...
namespace AvtoMigBussines.Detailing.Repositories.Implementations
{
    public class DetailingServiceRepository : IDetailingServiceRepository
    {
        ...
        public async Task<double?> GetSummAllServicesWithPrepayment(int? orderId)
        {
            var detailingOrderServicesSumm = await context.DetailingServices.
                Where(x => x.DetailingOrderId == orderId && x.IsDeleted == false).
                Select(x => x.Price).SumAsync();

            var detailingOrder = await context.DetailingOrders.FirstOrDefaultAsync(x => x.Id == orderId);

            return detailingOrderServicesSumm - detailingOrder.Prepayment;
        }
