    ...
    public class DetailingRepository : IDetailingRepository
    {
        ...
        public async Task<DetailingOrder> GetByIdForCompleteAsync(int id)
        {
            return await context.DetailingOrders.
                ...
                Where(x => x.IsDeleted == false && x.IsOvered == false).
                FirstOrDefaultAsync(p => p.Id == id);
        }

        public async Task<DetailingOrder> GetByIdForReadyAsync(int id)
        {
            return await context.DetailingOrders.Include(x=>x.ModelCar.Car).
                Where(x=>x.IsDeleted == false && x.IsOvered == false).
                Where(x=>x.IsReady == false).
